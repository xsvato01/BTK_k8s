...
def main():
  ...
  while line.startswith('#'):
    ...
    if len(lines) == header_lines_count:
      eprint("WARNING! VCF file contains no variants")
...
def process_line(line, ann_header, found_anno, t_pos, csq_pattern, clinvar_pattern, saaf_pattern, sapp_pattern):
  ...
  if "RD" in gth:
    ad_ref_pos = gth.index("RD") #varscan workaround
    add_variant['tumor_AD_ref'] = re.split(',', gtf[ad_ref_pos])[0]
    add_variant['tumor_AD_alt'] = re.split(',', gtf[ad_pos])[0]
  else:
